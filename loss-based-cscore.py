...
parser.add_argument('--logdir', default='orders3', type=str,
...
def main():
    tr_set = get_dataset(args.dataset, args.datadir, 'test',rand_fraction=args.rand_fraction)
    if args.dataset in ['food101','cifar10', 'cifar100', 'cifar100N']:
        tr_set_clean = tr_set
    else:
...
def subset_train(seed,tr_set,train_loader,val_loader,val_order,ind_loss,args):
    ...
    return validate(val_loader, model, nn.CrossEntropyLoss(reduction="none").to("cuda"),val_order,ind_loss)
