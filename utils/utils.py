...
def get_model(model_name, nchannels=3, imsize=32, nclasses=10, half=False):
  ...
  #   # )
  #   net.fc = nn.Linear(num_ftrs, nclasses)
  if imsize < 32 and model_name in models.__dict__:
    model = models.__dict__[model_name](num_classes=nclasses, nchannels=nchannels)
  else:
    print("=> using trochvision resnet50 as the model")
    assert model_name == 'resnet50', "only resnet50 is supported for imagenet"
...
def sample_with_distance(sorted_scores,scores, num_points, ordering, bw_factor = 1.0):

  query_point =  sorted_scores[num_points]
  
  #num_points = 2* np.count_nonzero(scores<query_point)
  
  bw = np.abs(sorted_scores[0]-query_point)/np.abs(sorted_scores[0]-sorted_scores[len(sorted_scores)//2])
  bw *= bw_factor
  
  #print('SWD',bw,query_point,num_points,sorted_scores[0],sorted_scores[len(sorted_scores)//2])
  if ordering != 'random':
    distance = np.exp(-1*((scores-query_point)**2)/(2*bw**4))
  else:
    distance = np.ones_like(scores)
    
  samples = np.random.choice(np.arange(len(scores)),2*num_points,p=distance/np.sum(distance),replace = False)

  return samples


def balance_sample_with_distance(sorted_order, scores, targets, num_classes, num_points, ordering, bw_factor = 1.0):

  num_points_class = num_points//num_classes
  sorted_order = np.asarray(sorted_order)
  samples = []
  for c in np.arange(num_classes):
    class_ind = np.where(targets==c)[0]
    scores_class = scores[class_ind]
    
    sorted_scores_class = scores[sorted_order[class_ind]]
    #print(targets[class_ind],sorted_scores_class)
    query_point =  sorted_scores_class[num_points_class]
  
    #num_points = 2* np.count_nonzero(scores<query_point)
    
    bw = np.abs(sorted_scores_class[0]-query_point)/np.abs(sorted_scores_class[0]-sorted_scores_class[len(sorted_scores_class)//2])
    bw *= bw_factor
    
    #print('SWD',2*num_points_class, len(scores_class), bw,query_point,num_points_class,sorted_scores_class[0],sorted_scores_class[len(sorted_scores_class)//2])
    if ordering != 'random':
      distance = np.exp(-1*((scores_class-query_point)**2)/(2*bw**4))
    else:
      distance = np.ones_like(scores_class)
    if 2*num_points_class>len(scores_class):
      num_points_class = len(scores_class)//2
      
    _samples = np.random.choice(np.arange(len(scores_class)),2*num_points_class,p=distance/np.sum(distance),replace = False)
    samples.extend(class_ind[_samples])
